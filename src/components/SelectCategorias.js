...
const SelectCategorias = ({categoria, cambiarCategoria}) => {
  ...
  const [mostrarSelect, cambiarMostarSelect] = useState(false);
  ...
  return (
    <ContenedorSelect onClick={() => cambiarMostarSelect(!mostrarSelect)}>
      <OpcionSeleccionada>
        {categoria} <IconoDown />
      </OpcionSeleccionada>
      ...
      {mostrarSelect && (
        <Opciones>
          {categorias.map((categoria) => (
            <Opcion key={categoria.id} data-valor={categoria.id} onClick={handleCick}>
